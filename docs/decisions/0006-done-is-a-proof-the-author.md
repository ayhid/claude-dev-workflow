# 0006. Done is a proof the author checks in a minute

- Status: superseded by [0007](0007-done-is-a-proof-the-author.md)
- Date: 2026-09-18
...
**And the author cannot run any of it.** `dw` — the binary a human types — exposes `init`, `update`,
...
hook rather than a hand. Asked to freeze this very record with `dev.mjs adr accept 6`, the author's
answer was that he can never launch commands of that kind. So the tool has exactly one operator, and
it is the agent. That is why the ADRs cannot be found, why the standup reads as noise — it only ever
arrives unbidden at session start, never on demand — and why every close happened somewhere the author
was not.
...
- **A proof the author checks himself, in about a minute, declared before the code exists** — chosen.

## Decision

**A ticket is not done until its author has checked its proof. The proof is declared at planning
time, in the author's words, and it is checkable in about a minute without reading code.**

Four rules follow, and they bind commands rather than prose:
...
5. **A command the author must run is a command the author can type.** One word from the project root,
   through `dw`, with no interpreter and no path: `dw standup`, `dw adr list`, `dw proof <ID>`. An
   unrecognised subcommand delegates to the project's `_dev-workflow/scripts/dev.mjs`. Rules 1 and 3
   are unimplementable without this, so it is built first.

## Consequences

The `dw` proxy is the first work item and the cheapest: one delegation branch in `bin/install.mjs`,
where the global binary today refuses what it does not recognise. Until it exists, every rule above
describes a workflow only the agent can operate, which is the condition this record was written to end.
...
This record supersedes nothing formally, but it reorders ADR 0004: the lifecycle it split stays split,
and each step now owes a proof at the end rather than a state change.

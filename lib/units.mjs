...
/**
 * The body a child issue is filed with: the unit's own text plus the
 * dependency line, when there is anything to depend on.
 *
 * @param {{description: string, dependsOn?: string[]}} unit
 */
export function renderUnitBody({ description, dependsOn = [] }) {
  const text = String(description ?? '').replace(/\s+$/, '');
  if (!dependsOn.length) return `${text}\n`;

...
export function coverageOf({ rules = DOCTRINE_RULES, resolved = {}, tools = [] } = {}) {
  ...
  return [...rules]
    ...
    .map((rule) => {
      ...
      if (usable.length === 0) {
        return { ...base, verdict: 'missing', expressible: false, evidence: [], reason: inexpressible(rule, configured) };
      }
...
/**
 * Why no linter in this project decides a rule.
 *
 * Three different situations, and a reader needs to be told which one: nothing
 * is configured here, what is configured cannot express it, or no linter can.
 * Only the second is a reason to look at another tool, and only the first is a
 * reason to install one.
 */
function inexpressible(rule, configured) {
  const could = DOCTRINE_TOOLS.filter((t) => rule.tools[t]);
  if (could.length === 0) return 'no linter can express this';

  const here = DOCTRINE_TOOLS.filter((t) => configured.has(t));
  if (here.length === 0) {
    return `no linter is configured here — ${could.join(' or ')} could decide it`;
  }
  return (
    `${here.join(' and ')} cannot express this (${here
      .map((t) => `${t}: ${rule.absent[t] ?? 'has no equivalent'}`)
      .join('; ')}); ${could.join(' or ')} could`
  );
}
...
/**
 * Every tool rule the coverage says to add, merged by rule id.
 *
 * **Two doctrine rules can want the same tool rule, and ESLint gives it one
 * entry.** `no-restricted-syntax` takes every selector in a single array, so
 * `result-types` and `boolean-behaviour-params` printed as two lines in one
 * `rules` object means the second silently replaces the first — a paste that
 * does half of what it claims, with nothing to notice. Merging here is what
 * makes the output pasteable; a reader who has to spot the collision is a
 * reader the report failed.
 *
 * @param {object[]} coverage
 * @param {{dialect: 'flat'|'eslintrc'|'biome'}} options
 */
export function collectSuggestions(coverage = [], { dialect } = {}) {
  const merged = new Map();

  for (const entry of [...coverage].sort((a, b) => a.id.localeCompare(b.id))) {
    const rule = DOCTRINE_RULES.find((r) => r.id === entry.id);
    if (!rule || rule.level === 'doctrine-only') continue;
    if (entry.expressible === false) continue;
    if (entry.verdict === 'covered' || entry.verdict === 'unknown') continue;

    const already = new Set(
      (entry.evidence ?? []).filter((e) => e.present === true && !e.weaker).map((e) => e.rule),
    );

    for (const tool of DIALECT_TOOLS[dialect] ?? []) {
      for (const tr of rule.tools[tool] ?? []) {
        if (already.has(tr.id)) continue;
        const found = merged.get(tr.id);
        if (!found) {
          merged.set(tr.id, {
            id: tr.id,
            ...
            options: [...tr.options],
            forRules: [entry.id],
            caveats: rule.caveat ? [rule.caveat] : [],
            needsProjectInput: Boolean(rule.needsProjectInput),
          });
          continue;
        }
        found.forRules.push(entry.id);
        if (rule.caveat) found.caveats.push(rule.caveat);
        found.needsProjectInput ||= Boolean(rule.needsProjectInput);
        // The stricter severity wins: a rule one doctrine rule wants as an
        // error is not softened by another that would settle for a warning.
        if (tr.severity === 'error') found.severity = 'error';
        for (const option of tr.options) {
          if (!found.options.some((existing) => JSON.stringify(existing) === JSON.stringify(option))) {
...
/** One merged suggestion, written in the dialect the project's config uses. */
function renderMerged(suggestion, dialect) {
  if (dialect === 'biome') {
    const [group, name] = suggestion.id.split('/');
    return `${JSON.stringify(group)}: { ${JSON.stringify(name)}: ${JSON.stringify(suggestion.severity)} },`;
  }
  const args = [suggestion.severity, ...suggestion.options];
  return dialect === 'eslintrc'
    ? `${JSON.stringify(suggestion.id)}: ${JSON.stringify(args)},`
    : `${jsLiteral(suggestion.id)}: ${jsLiteral(args)},`;
}

/** `  - name        value` — the two-column shape the rules report already uses. */
const bullet = (label, value, width = 15) => `  - ${String(label).padEnd(width)} ${value}`.trimEnd();

/** A tool's evidence for one doctrine rule, in one line. */
function evidenceLine(entry) {
  // A family where one member suffices reads as a contradiction otherwise:
  // "covered" above a line with two crosses in it looks like a bug.
  const suffix = entry.satisfiedBy === 'any' && entry.evidence.length > 1 ? '   (any one of these)' : '';
  return entry.evidence
    .map((e) => {
      if (e.present === 'unknown') return `${e.rule} ?`;
      if (e.present !== true) return `${e.rule} ✗`;
      return e.weaker ? `${e.rule} (found ${e.found}) — weaker than proposed` : `${e.rule} ✓`;
    })
    .join(' · ') + suffix;
}

/** How the stack block reads. */
function renderStack(stack) {
  const lines = [];
  if (stack.typescript?.present) {
    const strict = stack.typescript.strict === 'unknown'
      ? `strict: unknown (extends ${stack.typescript.extends} — not followed)`
      : `strict: ${stack.typescript.strict ? 'on' : 'off'}`;
    lines.push(bullet('typescript', `${stack.typescript.declared ?? 'declared by tsconfig'}   ${strict}`));
    for (const [flag, on] of Object.entries(stack.typescript.flags ?? {})) {
      if (on === false) lines.push(bullet('', `${flag}: off`));
    }
  }
  if (stack.frameworks?.length) lines.push(bullet('frameworks', stack.frameworks.join(', ')));
  // TypeScript already has its own line above; repeating it is noise, and in a
  // report that goes into a session's context noise has a price.
  const others = (stack.languages ?? []).filter((l) => !(l === 'TypeScript' && stack.typescript?.present));
  if (others.length) lines.push(bullet('declares', others.join(', ')));

  const ws = stack.workspaces ?? { kind: 'single' };
  lines.push(
    bullet(
      'packages',
      ws.kind === 'single'
        ? 'single package'
        : `${ws.tools.join(', ')} — ${ws.packages === 'unknown' ? 'package list unreadable' : ws.packages.join(', ')}`,
    ),
  );
  return lines.length ? lines : [bullet('', 'nothing here declares a stack this tool can name')];
}

/** How the tooling block reads. */
function renderTooling(tooling) {
  if (!tooling.length) return ['  - no linter is configured in this project'];

  return tooling.flatMap((tool) => {
    const named = tool.namedIn?.length
      ? `named in: ${tool.namedIn.map((n) => (n.via === 'direct' ? n.where : `${n.where} (${n.via})`)).join(', ')}`
      : 'named in: nothing';
    const lines = [
      bullet(tool.name, `${tool.configs.join(', ')}   ${tool.installed ? 'installed' : 'not installed'}   ${named}`),
    ];
    if (!tool.namedIn?.length) {
      lines.push(`      nothing in this repo runs ${tool.name} — a rule added here will never fail anything`);
    }
    if (tool.resolve?.ok === false) {
      lines.push(`      configuration not resolved: ${tool.resolve.reason}`);
    } else if (tool.resolve?.source === 'config') {
      lines.push(`      read from ${tool.configs[0]} (${tool.name} reports no resolved configuration)`);
    } else if (tool.resolve?.command) {
      lines.push(`      resolved by running: ${tool.resolve.command}`);
    }
    return lines;
  });
}

/** The verdict groups, in the order a reader wants them. */
const GROUPS = [
  { verdict: 'covered', heading: 'covered' },
  { verdict: 'partial', heading: 'partial' },
  { verdict: 'missing', heading: 'missing' },
  { verdict: 'unknown', heading: 'unknown — nothing is proposed for these' },
  { verdict: 'doctrine-only', heading: 'no linter decides these — they stay in the doctrine' },
];

/**
 * The coverage report.
 *
 * Pure, and it takes a plain object rather than a directory: the repo path
 * stays in the caller's prefix line, so nothing here is a path, a timestamp or
 * a duration and the same project prints the same bytes twice (rule 4). That is
 * what makes the report a thing you can diff, which is what makes a second run
 * proving nothing changed cost one command.
 *
 * @param {{stack: object, tooling: object[], coverage: object[], summary: object}} input
 */
export function renderDoctrine({ stack = {}, tooling = [], coverage = [], summary = {} } = {}) {
  const sorted = [...coverage].sort((a, b) => a.id.localeCompare(b.id));
  const width = Math.max(0, ...sorted.map((r) => r.id.length));
  const out = [];

  out.push('stack', ...renderStack(stack), '');
  out.push('verification tooling', ...renderTooling(tooling), '');

  const counted = `${summary.covered ?? 0} covered, ${summary.partial ?? 0} partial, ` +
    `${summary.missing ?? 0} missing, ${summary.unknown ?? 0} unknown, ` +
    `${summary.doctrineOnly ?? 0} doctrine-only`;
  out.push(`doctrine coverage — ${sorted.length} rules: ${counted}`);

  for (const group of GROUPS) {
    const rules = sorted.filter((r) => r.verdict === group.verdict);
    if (!rules.length) continue;
    out.push('', `  ${group.heading}`);
    for (const rule of rules) {
      const detail = rule.evidence.length ? evidenceLine(rule) : rule.reason;
      out.push(`    - ${rule.id.padEnd(width)}  ${rule.level.padEnd(13)} ${detail}`);
    }
  }

  // A tool is asked for its dialect once, and every snippet is written in it.
  const dialect = tooling.find((t) => t.dialect)?.dialect ?? 'flat';
  const suggestions = collectSuggestions(sorted, { dialect });

  if (suggestions.length) {
    out.push('', `suggestions (${suggestions.length}) — nothing below has been written. Run the count first.`);
    if (dialect === 'eslintrc') {
      out.push(
        '  this project is on a legacy eslintrc config. The rules below are written for it and',
        '  are not migrated — moving to flat config is a separate decision this does not make.',
      );
    }
    for (const suggestion of suggestions) {
      out.push('', `  ${suggestion.id}   for: ${suggestion.forRules.join(', ')}`);
      for (const caveat of suggestion.caveats) out.push(`    caveat: ${caveat}`);
      if (suggestion.needsProjectInput) {
        out.push('    this one needs a value only the project can supply — the placeholder is not a default');
      }
      out.push(`      ${renderMerged(suggestion, dialect)}`);
    }
  }

...
/**
 * Paths excluded from a review diff by default: lockfiles, build output and
 * generated source. Reviewing them wastes the reviewer's attention and, for a
 * model-driven review, most of the context budget.
 */
export const DEFAULT_DIFF_EXCLUDES = [
  // `.lock$` alone is the trap: it catches Cargo.lock, yarn.lock, poetry.lock and
  // flake.lock, and misses package-lock.json and pnpm-lock.yaml — the two that
  // actually run to thousands of lines and push a dependency bump past the
  // reviewable ceiling. Name them.
  /\.lock$/,
  /(^|\/)(package-lock|npm-shrinkwrap)\.json$/,
  /(^|\/)pnpm-lock\.ya?ml$/,
  // Anchored at any path segment, not just the repo root: in a monorepo the build
...
export function makeVcs({ run }) {
  ...
  /**
   * The freshest `base` this checkout can see: `<remote>/<base>` after a
   * fetch when the remote exists and the fetch lands, else the local branch.
   *
   * The local base is where a shared root checkout was left, which on a
   * long-lived checkout is always behind. Forking a ticket from it, or landing
   * onto it, is how a clean fast-forward turns into someone else's revert
   * (#122). `start`, `build --start` and `landDirect` all ask here, so the
   * question has one answer. The local branch is never moved: `fetch` updates
   * the remote-tracking ref only, and a root checkout other sessions share
   * stays exactly where they left it.
   *
   * A fetch that fails is not fatal — offline, the local base is the freshest
   * there is — but `why` says so, and every caller prints it.
   *
   * @returns {Promise<{ref: string, hasRemote: boolean, fetched: boolean, why: string|null}>}
   */
  async function freshestBase({ dir, remote = 'origin', base }) {
    const hasRemote = (await git(dir, ['remote', 'get-url', remote])).ok;
    if (!hasRemote) return { ref: base, hasRemote, fetched: false, why: `no remote "${remote}"` };
    const tracking = `${remote}/${base}`;
    const f = await git(dir, ['fetch', remote, base]);
    if (f.ok && (await refExists(dir, tracking))) return { ref: tracking, hasRemote, fetched: true, why: null };
    const reason = (f.stderr || f.stdout || '').trim().split('\n').pop() || `${tracking} does not exist after the fetch`;
    return { ref: base, hasRemote, fetched: false, why: `could not fetch ${tracking}: ${reason}` };
  }
  ...
  async function landDirect({ repoDir, workDir, branch, base, remote = 'origin', push = true }) {
    ...
    for (const dir of new Set([workDir, repoDir])) {
      ...
      if (!clean.clean) {
        return {
          ok: false,
          error: `${dir} has uncommitted changes — commit them before landing:\n  ${clean.dirty.join('\n  ')}`,
        };
      }
    }

    // Rebase onto the freshest base we can see — see freshestBase.
    const fresh = await freshestBase({ dir: repoDir, remote, base });
    const { ref: onto, hasRemote } = fresh;
    steps.push(fresh.fetched ? `fetch ${remote} ${base}: ok` : `fetch ${remote} ${base}: ${fresh.why}`);
  ...
  /**
   * What `branch` changed relative to `base` — patch, file list and size, in one call.
   *
   * Three-dot on purpose. `base..head` reports everything that differs between the
   * two tips, so commits landed on the base since the branch started read as
   * *reversed* changes in the branch's own diff. `base...head` is the change the
   * branch actually made, which is the only thing a reviewer should be shown.
   *
   * `exclude` drops paths nobody reviews — lockfiles, build output, generated
   * source. They are excluded from the file list and the patch alike, so the
   * line count reflects reviewable work rather than a regenerated lockfile.
   *
   * @returns {Promise<{ok: true, patch: string, files: string[], lines: number}
   *                 | {ok: false, error: string}>}
   */
  async function diffRange(dir, { base, head = 'HEAD', exclude = DEFAULT_DIFF_EXCLUDES } = {}) {
    if (!base) return { ok: false, error: 'diffRange needs a base ref' };
    if (!(await refExists(dir, base))) return { ok: false, error: `no such ref "${base}" in ${dir}` };
    if (!(await refExists(dir, head))) return { ok: false, error: `no such ref "${head}" in ${dir}` };

    const range = `${base}...${head}`;
    const names = await git(dir, ['diff', '--name-only', range]);
    if (!names.ok) return { ok: false, error: names.stderr || `could not diff ${range} in ${dir}` };

    const keep = (f) => f && !exclude.some((re) => re.test(f));
    const files = (names.stdout ? names.stdout.split('\n') : []).filter(keep);

    // Nothing reviewable is not an error; it is an empty review.
    if (files.length === 0) return { ok: true, patch: '', files: [], lines: 0 };

    const patch = await git(dir, ['diff', range, '--', ...files]);
    if (!patch.ok) return { ok: false, error: patch.stderr || `could not diff ${range} in ${dir}` };

    const stat = await git(dir, ['diff', '--numstat', range, '--', ...files]);
    let lines = 0;
    if (stat.ok && stat.stdout) {
      for (const row of stat.stdout.split('\n')) {
        const [a, d] = row.split('\t');
        // A binary file reports "-\t-". Counting it as zero is right: there are
        // no reviewable lines in it.
        lines += (Number(a) || 0) + (Number(d) || 0);
      }
    }
    return { ok: true, patch: patch.stdout, files, lines };
  }

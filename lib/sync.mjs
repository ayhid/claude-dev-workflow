 ...
 *
 * `observations` are `{ id, rank, state, url, at? }`. A merged PR (higher rank)
 * beats an open one for the same issue, so a ticket with both lands on `done`.
 *
 * `at` is when the evidence happened — a merged PR's `mergedAt` — and is carried
 * only when the observation had one: the metrics log dates an observed close at
 * it (#48), and an open PR or a landed commit has nothing to say. Absent rather
 * than null, so the shape every caller already matches on is unchanged.
 *
 * @returns {Map<string, {rank: number, state: string, url: string, at?: string}>}
 */
export function strongestEvidence(observations) {
  ...
  for (const o of observations) {
    ...
    if (!prev || o.rank > prev.rank) {
      const ev = { rank: o.rank, state: o.state, url: o.url };
      if (o.at != null) ev.at = o.at;
      out.set(o.id, ev);
    }
  }
  return out;
...
/**
 * The commits in a `git log` rendering of the base branch, in the order git
 * printed them (newest first), each with the issues its subject names.
 *
 * The one parse of that log. `sync` folds it into evidence below and `standup`
 * reports it as what landed (#44); two readers of one format is how the two
 * commands would come to disagree about which commits exist.
 *
 * Pure: `log` is the captured stdout, one `<sha>{LOG_SEP}<subject>` per line.
 * A line with no separator is skipped. A commit naming no issue is kept with
 * an empty `ids`, and each caller decides what that means to it — nothing to
 * reconcile, nothing to list.
 *
 * @param {string} log
 * @param {{syntax: unknown}} opts
 * @returns {{sha: string, subject: string, ids: string[]}[]}
 */
export function landedCommits(log, { syntax }) {
  const out = [];
  for (const line of String(log ?? '').split('\n')) {
    ...
    if (!sha) continue;
    out.push({ sha, subject, ids: extractIssueIds(subject, syntax) });
  }
  return out;
}
 ...
 *
 * `urlFor` renders a sha into whatever the forge calls that commit. Commits
 * whose subject names no issue contribute nothing — the same
...
export function commitObservations(log, { syntax, rank, state, urlFor }) {
  const out = [];
  for (const { sha, ids } of landedCommits(log, { syntax })) {
    for (const id of ids) out.push({ id, rank, state, url: urlFor(sha) });
  }
  return out;
}

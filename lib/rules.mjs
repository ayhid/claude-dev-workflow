...
export function renderRules({ linters = [], checks = [], sources = [], claims = [], languages = [] } = {}) {
  ...
  if (linters.length) {
    ...
    // Refusal 3, and the reason it is a refusal: there is nothing to propose a
    // rule *into*, so the answer is the name of the tool and not a config file.
    const standard = sortBy(
      [...new Set(languages.map((lang) => STANDARD_LINTERS[lang]).filter(Boolean))].map((name) => ({ name })),
      'name',
    );
    L.push(bullet('no linter is configured in this project'));
    if (standard.length) {
      L.push(bullet(`the standard one here would be: ${standard.map((s) => s.name).join(', ')} — installing it is a decision, not a proposal`));
    }
  }

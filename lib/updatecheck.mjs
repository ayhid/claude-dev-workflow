...
import { MANIFEST_PATH, PAYLOAD_DIR, compareVersions, resolveInstallRoots } from './manifest.mjs';
...
/**
 * The cache file for an install: beside the payload the answer is about.
 *
 * Local mode resolves to `<project>/CACHE_PATH`, exactly as before. Global mode
 * puts it under the machine root, one per machine: every project on the machine
 * calls the same runtime, so a cache per project would look the same answer up,
 * and announce it, once per project. Resolved through `resolveInstallRoots`
 * rather than spelled here — the machine root has one spelling, in manifest.mjs.
 */
export function cacheFile({ root, mode = 'local', env = process.env } = {}) {
  return join(resolveInstallRoots({ projectDir: root, mode, env }).payloadRoot, '_config', 'updatecheck.json');
}
 ...
 */
export function readCache(root, { readFile = readTextFile, file = join(root, CACHE_PATH) } = {}) {
  let raw;
  try {
    raw = readFile(file);
  } catch {
 ...
 */
export function writeCache(root, record, { writeFile = writeTextFile, file = join(root, CACHE_PATH) } = {}) {
  try {
    writeFile(file, `${JSON.stringify(record, null, 2)}\n`);
    return true;
  } catch {
    return false;
  }
 ...
 * @param {string} [opts.mode]       `install.mode` — decides where the cache lives
...
export async function checkForUpdate({
  ...
  // Where the cache lives follows the install mode. A mode nothing can resolve —
  // a typo, or global with no HOME — is a failed check like any other: silence.
  let file;
  try {
    file = cacheFile({ root, mode, env });
  } catch {
    return null;
  }

  const read = readCache(root, { readFile, file });
  ...
  if (read.corrupt) {
    writeCache(root, { latest: null, checkedAt: 0, announced: null }, { writeFile, file });
    return null;
  }
  ...
  if (!text) {
    ...
    if (!fresh) writeCache(root, { latest, checkedAt, announced: latest }, { writeFile, file });
    return null;
  }
  ...
  // extend the window.
  if (announced !== latest && !writeCache(root, { latest, checkedAt, announced: latest }, { writeFile, file })) return null;

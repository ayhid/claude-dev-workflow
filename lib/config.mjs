...
export function findConfigFile(startDir) {
  ...
  for (;;) {
    for (const rel of ['.youtrack.json', join('.claude', 'youtrack.json')]) {
...
export function projectRootFor(configFile) {
  ...
  // .claude/youtrack.json sits one level below the root.
...
export function formatConfig(config, file) {
  ...
  push('instance:', config.baseUrl ?? 'MISSING — run /yt-init');
  push('project:', config.project ?? 'MISSING — run /yt-init');

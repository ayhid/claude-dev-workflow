...
export const DEFAULTS = {
  ...
  /**
   * Which shipped hooks are active, one key per hook.
   *
   * One vocabulary for a question that used to be asked twice. `commit.enforce`
   * and `docs.enforce` each turned off their own guard, sitting beside the
   * settings that guard already read. That worked while every hook belonged to
   * an existing config block, and stopped working the moment one did not:
   * `sessionStart` runs the standup and owns no other settings, so its switch
   * had nowhere to live.
   *
   * The two older spellings are still honoured, and must stay that way — they
   * are documented, and a project that turned a guard off does not expect an
   * update to turn it back on. `false` in *either* place disables the hook. The
   * older key can only disable, never re-enable, so the two cannot contradict
   * each other into a question about precedence.
   *
   * A hook is on unless something says `false`. Missing means on, which is what
   * lets an on-by-default guard apply to a config written before it shipped.
   */
  hooks: {
    sessionStart: true,
    commitTicket: true,
  ...
  /**
   * Whether `/dev-build` drives implementation through `/dev-tdd`.
   *
  ...
  /**
   * Which install shape this project is on.
   *
   * `local` vendors the runtime into `_dev-workflow/` — what every install has
   * always done, and still the default, so a config written before this key
   * existed resolves to the shape it already has. `global` keeps the runtime
   * once on the machine, at the root `lib/manifest.mjs` names, and leaves the
   * project carrying nothing but its own configuration. The path is not
   * restated here: prose is a second spelling too, and it is the one nothing
   * would ever update.
   *
   * Read from the file rather than inferred from whether `_dev-workflow/`
   * exists: that directory is absent for a dozen reasons — a fresh clone of a
   * globally-installed project, a checkout that has not been installed yet —
   * and each of them would be read as a mode nobody chose (rule 2).
   *
   * `lib/manifest.mjs#resolveInstallRoots` turns this into paths. It is
   * deliberately **not** in `bin/lib/config-keys.mjs`: that registry prompts for
   * keys a config lacks, and an express `update` must never ask a question.
   */
  install: { mode: 'local' },
 ...
 *
 * `notesFileContent` and `roots` are both passed in rather than worked out
 * here: this function is pure and is called from tests with no filesystem and
 * no environment. The caller does the IO — see `scripts/cmd/config.mjs`.
 */
export function formatConfig(config, file, notesFileContent = null, roots = null) {
  const L = [];
  const push = (label, value) => L.push(`${label.padEnd(13)}${value}`);

  L.push(`config file: ${file ?? '<none — using defaults>'}`);
  // Where the tooling itself lives, before anything about the tracker: in
  // global mode `dev.mjs` is not under the project at all, and a session that
  // cannot see that has no way to know which path its own commands resolve to.
  //
  // The mode comes from the config, so it always prints; the roots are resolved
  // by the caller and passed in, because resolving them needs the environment
  // and this function does no IO — the same split as the notes file above.
  push('install:', config.install?.mode ?? DEFAULTS.install.mode);
  if (roots) {
    L.push(`             payload: ${roots.payloadRoot}`);
    L.push(`             skills:  ${roots.skillsRoot}`);
  }
  ...
  // /dev-build §4 branches on this, and reads it here rather than opening the

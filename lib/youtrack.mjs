...
export function createYouTrackProvider({ config, fetch: fetchImpl, onWarn }) {
  ...
  const provider = {
    ...
    capabilities: {
      ...
      freeTextSearch: true,
      rawCommand: true,
      // YouTrack has no repository template concept. The shipped defaults in
      // lib/issuetemplate.mjs apply here exactly as on GitHub; only the *repo*
      // lookup is what this declines.
     ...
     */
    /** No repository templates here — an empty answer, and no request made. */
    async templates() {
      return { ok: true, data: [] };
    },

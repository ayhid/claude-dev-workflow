 ...
 *
 * `observed` marks a close this machine did not make — one `sync` read off the
 * tracker after CI or a colleague moved the ticket (#48). It is the **last**
 * key and is written only when true, so a row a command wrote is byte-identical
 * to what it has always been, and a reader that does not know the key reads
 * old and new logs alike.
 *
 * @param {{
 *   role: string, id: string, state: string, at?: Date, provider?: string,
 *   elapsedMs?: ?number, starts?: ?number, criteria?: ?string, observed?: boolean,
 * }} event
 */
export function renderEvent({
  ...
  if (CLOSING.has(role)) {
    ...
    if (observed) line.observed = true;
  }

...
import { noPrExpected, PR_UNKNOWN } from './status.mjs';
...
/**
 * What landed on a base branch without a pull request, one entry per ticket.
 *
 * Under `delivery.mode: direct` nothing is ever merged — it is landed, by
 * `dev.mjs land`, straight onto the base — so the merged-PR section was blind
 * to the whole delivery path (#44). This is the same evidence `sync` reads
 * for the same reason (#19), reported rather than reconciled.
 *
 * `commits` is `landedCommits` output, newest first as git printed it. A
 * ticket named by several commits is one line — the newest sha and subject,
 * with a count of the rest — because the section answers "what landed", not
 * "how many commits did it take". A commit naming no ticket is dropped
 * outright: on a base branch that is every release bump and every chore, and
 * a "no ticket" line per commit would bury the section it is meant to fill.
 *
 * Same shape as a merged PR entry, so the renderer has one row format.
 * `mergedAt` is null: the log read carries no date, and the order is git's.
 *
 * The bound is the one `sync` accepted in #19: a squash-merged dependabot
 * subject carries its PR number as `(#N)`, and that is credited to issue #N.
 *
 * @param {{sha: string, subject: string, ids: string[]}[]} commits
 * @returns {{label: string, title: string, issueId: string, mergedAt: null, more: number}[]}
 */
export function landedSince(commits) {
  const byId = new Map();
  for (const c of commits ?? []) {
    for (const id of c.ids ?? []) {
      const seen = byId.get(id);
      if (seen) seen.more += 1;
      else byId.set(id, { label: String(c.sha).slice(0, 7), title: c.subject, issueId: id, mergedAt: null, more: 0 });
    }
  }
  return [...byId.values()];
}
...
/**
 * The rows that count as in flight: everything that is not a base branch.
 *
 ...
 *
 * Two bases, because a project can fork from one branch and deliver onto
 * another: `base` is where ticket branches fork from, and a row's own `base` is
 * where its repo lands work (#44). A checkout sitting on either is not work —
 * before this, a repo delivering onto `develop` listed `develop` in flight.
 *
...
export function inFlight(rows, { base }) {
  return (rows ?? []).filter((row) => row.branch !== base && (!row.base || row.branch !== row.base));
}
 ...
 *
 * `direct` says whether any repo delivers without a pull request (#44). The
 * first section is then "landed since" — merged PRs and landed commits alike,
 * since a PR opened by hand on a direct repo is evidence whatever the mode —
 * and a row on such a repo shows `direct` where a pr repo shows `none`. With
 * every repo on `pr` the report is byte-for-byte what it was.
 *
 * `landedUnread` is one line per direct repo whose base branch could not be
 * read — no such branch anywhere, or `git log` failed. The same rule as the
 * tracker section: not read and read-and-empty are two facts, and they must
 * never render the same, because the empty line's remedy — widen the window —
 * is exactly wrong when the read itself failed.
 *
 * @param {{
 ...
 *   now?: Date, prUnknown?: boolean, direct?: boolean, landedUnread?: string[],
 * }} facts
 */
export function describeStandup(facts) {
  ...
  const verb = direct ? 'landed' : 'merged';
  L.push('', `${verb} since ${cutoff.slice(0, 10)}`);
  if (!merged.length && !landedUnread.length) {
    L.push(`  (nothing ${verb} in the window — widen it with --since 7d)`);
  } else {
    for (const entry of merged) {
      const ticket = entry.issue ? `${entry.issue.id} ${entry.issue.state ?? 'unknown state'}` : 'no ticket';
      const more = entry.more > 0 ? `  (+${entry.more} more)` : '';
      L.push(`  ${entry.label ?? `#${entry.number}`}  ${truncate(entry.title, 52)}   ${ticket}${more}`);
      if (entry.issue?.state && rankOf(config, entry.issue.state) >= 0 &&
          rankOf(config, entry.issue.state) < rankOf(config, config.states.done)) {
        // The hint applies to both kinds, and on a direct repo this is the only
        // place it can live: the branch is gone once the work has landed, so no
        // in-flight row is left to carry the priority-0 signal.
        const kind = entry.number == null ? 'landed' : 'merged';
        L.push(`        ^ ${kind}, but the ticket has not been reconciled — dev.mjs sync --apply`);
      }
    }
  }
  // After whatever was read, so a section that is half read never looks whole.
  for (const note of landedUnread) L.push(`  (could not read what landed — ${truncate(note, 90)})`);
    ...
    for (const row of [...rows].sort(byRowId)) {
      L.push(
        `  ${(row.issue?.id ?? '-').padEnd(8)} ${(row.issue?.state ?? '-').padEnd(13)} ` +
          `${prCell(row.pr, row.delivery).padEnd(11)} ${treeCell(row).padEnd(8)} ` +
...
// The header stays `PR` whatever the mode: a monorepo can mix modes, and the
// cell is where the difference lives. `noPrExpected` is `status`'s own rule,
// checked before `PR_UNKNOWN` on purpose — without the GitHub CLI a direct
// repo still expects no PR, and `-` would say the answer is unknown when it
// is not.
const prCell = (pr, delivery = null) => {
  if (noPrExpected(pr, delivery)) return 'direct';

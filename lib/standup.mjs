 ...
 *   now?: Date, prUnknown?: boolean, direct?: boolean,
 * }} facts
 */
...
// cell is where the difference lives. `direct` is checked before `PR_UNKNOWN`
// on purpose — without the GitHub CLI a direct repo still expects no PR, and
// `-` would say the answer is unknown when it is not.
const prCell = (pr, delivery = null) => {
  if (delivery === 'direct' && (!pr || pr === PR_UNKNOWN)) return 'direct';

...
export function mergeSources(ledger, found, { now = new Date(), exists = () => false } = {}) {
  ...
  // marked stale, since there is no partial-truth reading of "this document is
  // still worth keeping" the way a claim can be half-right, and a document that
...
export function nextUnit(ledger) {
  ...
  // A document a verdict has set aside — `archive` or `delete`, see
  // lib/reorg.mjs — is not read: claims from a record of a moment are the
  // year-old-snapshot failure the anchoring rule exists to prevent. Never
  // silently, though. The count travels with the unit so the caller can say
  // how many were passed over, and `describeLedger` shows it too.
  const aside = setAsidePaths(ledger);
  const pending = sources.filter((s) => s.state === 'pending');
  const unread = pending.filter((s) => !aside.has(s.path)).sort(byPath);
  const skipped = pending.length - unread.length;
  if (unread.length) {
    return {
      phase: 'extract',
      what: `read ${unread[0].path} and record what it claims${skipped ? ` (${skipped} set aside)` : ''}`,
      detail: { path: unread[0].path, remaining: unread.length, pending: unread.map((s) => s.path), skipped },
    };
  }
...
/** Paths whose relevance verdict says they are not to be read as sources. */
const SET_ASIDE = ['archive', 'delete'];
const setAsidePaths = (ledger) =>
  new Set((ledger.verdicts ?? []).filter((v) => SET_ASIDE.includes(v.classification)).map((v) => v.path));
...
export function describeLedger(ledger) {
  ...
  const aside = setAsidePaths(ledger);
  const setAside = count(sources, (s) => aside.has(s.path));
  if (setAside) L.push(`set aside: ${setAside} (archive or delete verdict — never offered for reading)`);
...
/**
 * A list of paths as one bounded line: the count, the first few names in
 * code-unit order, and how many were left out.
 *
 * `scan` is the first command a brownfield survey runs, and its output goes
 * into the session's context, where a token added early is paid for on
 * every turn after (#61: one `gone:` line of 81,570 characters). The full
 * list is in the ledger, which is where a caller that wants all of it reads.
 * `SUMMARISED_PATHS` is exported so the caller that mentions the ledger when
 * a list was cut can agree with the cut.
 */
export const SUMMARISED_PATHS = 5;

export function summarisePaths(paths, { max = SUMMARISED_PATHS } = {}) {
  const sorted = [...paths].sort();
  if (!sorted.length) return '0';
  const shown = sorted.slice(0, max);
  const rest = sorted.length - shown.length;
  return `${sorted.length} — ${shown.join(', ')}${rest ? `, … ${rest} more` : ''}`;
}

...
export function classifyPath(path) {
  ...
  //
  // `.claude/skills/dev-*` is the same files again — the installed copy of the
  // shipped skills — so surveying both reads every one of them twice and
  // manufactures a contradiction between a file and itself. The boundary is the
  // same one `isOwnedPath` draws: `dev-` prefixed only, because a skill the
  // user wrote is genuinely their documentation.
  if (clean.startsWith('_dev-workflow/')) return 'other';
  if (/^\.claude\/skills\/dev-[^/]+\//.test(clean)) return 'other';
...
export function mergeSources(ledger, found, { now = new Date() } = {}) {
  ...
  return {
    ledger: { ...ledger, sources: sources.sort(byPath), claims },
    changed,
    gone,
 ...
 */
export function validateClaim(claim, { id } = {}) {
 ...
 * for a complete one.
 */
export function addClaims(ledger, incoming) {
  ...
  for (const raw of incoming ?? []) {
    const result = validateClaim(raw, { id: `c${next}` });
    if (!result.ok) {

...
`claude-dev-workflow init` — the binary from `brew install claude-dev-workflow` or
...
- Give them the command it printed — `npx claude-dev-workflow@latest --update`, or
  `dev.mjs version --upgrade`, or `claude-dev-workflow update` where the binary is installed.

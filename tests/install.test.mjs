...
test('every skill is installed under its dev- name', () => {
  const files = [...planFiles(SOURCE_ROOT).keys()];
  for (const skill of ['dev-task', 'dev-file', 'dev-plan', 'dev-split', 'dev-build', 'dev-bug', 'dev-done', 'dev-init', 'dev-standup', 'dev-ingest-docs', 'dev-docs-init', 'dev-adr', 'dev-tdd', 'dev-review', 'dev-lint-rules']) {
    assert.ok(
...
test('a fresh install writes the payload, the skills and the manifest', () => {
  ...
  assert.deepEqual(manifest.skills.sort(), [
    'dev-adr', 'dev-bug', 'dev-build', 'dev-docs-init', 'dev-done', 'dev-file', 'dev-ingest-docs', 'dev-init', 'dev-lint-rules', 'dev-plan', 'dev-review', 'dev-split', 'dev-standup', 'dev-task', 'dev-tdd',
  ]);

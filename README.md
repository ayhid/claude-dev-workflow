...
| `/dev-review` | Three adversarial passes over the branch diff, each with a **different payload**: one blind pass that never sees the ticket, one hunting the input that breaks it, one auditing code against intent *and intent against itself*. Findings sorted into fix-the-code, fix-the-spec, out-of-scope. **Never edits, never lands.** [Findings contract →](docs/review.md) |
| `/dev-lint-rules` | Turns the conventions a project only *states* — in `CLAUDE.md`, `CONTRIBUTING.md`, the docs ledger — into rules its own linter can decide, each presented with the count of existing violations it would flag. Also reports which of the clean-code doctrine's mechanically checkable rules that linter **already** decides, by resolving its real configuration rather than guessing from a config file, and suggests the rest inside the tool already in place. What no rule can decide it reports separately, as a hook, a claim, or noise. **Writes nothing without approval of the batch.** [Why not a document →](docs/documentation.md#why-there-is-no-conventionsmd) |
...
| A rule the project already has is never proposed again, and a rule nothing could tell us about is never proposed at all. | `dev.mjs rules --doctrine` runs the linter's own `--print-config` and reads the verdict off it. A resolve that could not run — a linter declared but not installed — reports `unknown`, which is not `missing`: proposing a rule a project already has is the one failure the command exists to prevent. |
| A linter that nothing runs is called out before any rule is chosen for it. | The report's column is headed `named in`, never `runs in`: a word match proves the name appears in a script, a hook or a CI job, and a commented-out step matches too. The weaker claim is the true one. |
...
your-project/
  ...
  .claude/
    ...
    agents/dev-reader.md          # the subagents the skills dispatch, one file each
           ...
           dev-builder.md
    settings.json                 # the five hooks, merged in alongside your own
```

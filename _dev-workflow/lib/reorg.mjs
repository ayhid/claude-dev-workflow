...
import { adrFilename, nextNumber, renderAdr } from './adr.mjs';
...
// ---------------------------------------------------------------------------
// Intent claims as proposed decision records
// ---------------------------------------------------------------------------

/** The longest title a proposed record gets; past it the first sentence is cut and marked. */
const TITLE_MAX = 100;

/**
 * The first sentence of a claim, as a title. Cut at the first sentence end,
 * then at `TITLE_MAX` with an ellipsis, so a claim written as a paragraph
 * still yields a heading rather than a heading-sized wall.
 */
function titleOf(text) {
  const one = String(text ?? '').replace(/\s+/g, ' ').trim();
  const m = /^(.+?[.!?])(\s|$)/.exec(one);
  const sentence = m ? m[1] : one;
  return sentence.length <= TITLE_MAX ? sentence : `${sentence.slice(0, TITLE_MAX - 1).trimEnd()}…`;
}

/**
 * One proposed ADR per live `intent` claim.
 *
 * `ingest` already separates what is checkable against the tree from what
 * only a person can know, and the second kind — *why* something is the way it
 * is — is exactly what a decision record holds. This joins the two rather
 * than leaving them neighbours: the claim's text becomes the record's title
 * and context, quoted with its provenance, and `/dev-adr` is what fills in
 * the options and accepts or rejects it. Nothing here decides anything.
 *
 * Numbered from `nextNumber(existingNumbers)` — the project's *real* decisions
 * directory, read by the command — so a proposal accepted by copying it over
 * never collides with a record already there. Dated from the claim's
...
export function proposedAdrs(ledger, { existingNumbers = [] } = {}) {
  ...
  for (const claim of intents) {
    ...
    const context = [
      `Proposed from claim \`${claim.id}\`, an intent recorded by \`dev.mjs ingest\` from ${from}` +
        `${claim.topic ? ` under the topic *${claim.topic}*` : ''}. It says why, not what — nothing in the tree can settle it, so it is a decision to accept, reject or supersede:`,
      '',
      ...String(claim.text).trim().split('\n').map((line) => `> ${line}`),
    ].join('\n');
    adrs.push({
      number,
      file: adrFilename(number, title),
      claimId: claim.id,
      source: claim.source ?? null,
      text: renderAdr({
        number,
        title,
        date: String(claim.recordedAt ?? '').slice(0, 10),
        status: 'proposed',
        context,
        options: [],
        consequences: null,

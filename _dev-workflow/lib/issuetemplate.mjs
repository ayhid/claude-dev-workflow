...
/**
 * Every file parsed on its own: the readable ones as templates, the rest as
 * `{filename, error}`. One broken file must not take the others down — a repo
 * with a valid `bug_report.md` beside a `feature.yml` missing a `type:` can
 * still file bugs — but a directory whose every template is unreadable is an
 * error, not an empty directory: the project stated a shape, and quietly
 * filing the shipped skeleton instead would be the substitution this module
 * exists to stop.
 */
const parseAll = (files) => {
  ...
  for (const f of files) {
    ...
    else unreadable.push({ filename: f.filename, error: r.error });
  }
  if (!templates.length && unreadable.length) {
    return { ok: false, error: `no readable issue template — ${unreadable.map((u) => u.error).join('; ')}` };
  }
  return { ok: true, templates, unreadable };
};
 ...
 * `unreadable` lists the files that were there but could not be parsed, for
 * the caller to warn about; naming one with `--template` gets its error.
 *
 * @returns {Promise<{ok: true, templates: object[], unreadable: Array<{filename: string, error: string}>, source: 'checkout'|'api'|'none'} | {ok: false, error: string}>}
 */
export async function discoverTemplates({ root, provider }) {
  const local = parseAll(discoverLocal(root));
  if (!local.ok) return local;
  if (local.templates.length) return { ok: true, templates: local.templates, unreadable: local.unreadable, source: 'checkout' };

  if (!provider?.capabilities?.issueTemplates) return { ok: true, templates: [], unreadable: [], source: 'none' };
  ...
  return remote.templates.length
    ? { ok: true, templates: remote.templates, unreadable: remote.unreadable, source: 'api' }
    : { ok: true, templates: [], unreadable: [], source: 'none' };
}
 ...
 */
export function selectTemplate({ templates = [], source = 'none', name, type, unreadable = [] } = {}) {
  if (templates.length) {
    if (name !== undefined) {
      ...
      if (!hit) {
        // The one asked for exists and could not be read: say why, not "not found".
        const broken = unreadable.find((u) => u.filename.toLowerCase() === want || stem(u.filename).toLowerCase() === want);
        if (broken) return { ok: false, error: `issue template "${name}" could not be read — ${broken.error}` };
        return { ok: false, error: `no issue template named "${name}" — the repository has: ${listNames(templates)}` };
      }
...
/**
 * The body split at its headings: which sections it has, and what is under
 * each. A section is a `##` or `###` heading here as it is in the template
 * reader — the validator holds the body to the same rule its templates were
 * read by, so `#### Acceptance criteria` does not satisfy a check the
 * template file itself could not have defined. Deeper headings are content.
 */
function sectionsOf(body) {
  const lines = String(body ?? '').split(/\r?\n/);
  const heads = headingsOf(body).filter((h) => h.level === 2 || h.level === 3);
...
/**
 * Is there anything under a criteria heading beyond what `renderTemplate`
 * printed there? `- [ ] AC1: ` is the shipped placeholder, and a checkbox
 * with nothing after it is the empty section wearing a checkbox.
 */
const hasCriteria = (content) =>
  String(content ?? '')
    .split(/\r?\n/)
    .some((line) => line.trim() && !/^\s*[-*]\s*\[[ xX]?\]\s*(?:AC\d+\s*:)?\s*$/.test(line));
...
export function validateBody(body, template, { source = 'default' } = {}) {
  ...
  if (source === 'default') {
    for (const s of template.sections) {
      const got = find(s);
      if (isCriteria(s)) {
        if (!got || !hasCriteria(got.content)) missing.push(s);
      } else if (!got) {

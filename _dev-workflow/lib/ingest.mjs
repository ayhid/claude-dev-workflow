...
export function mergeSources(ledger, found, { now = new Date(), exists = () => false } = {}) {
  ...
  for (const file of found) {
    ...
    // An excluded path back in the corpus is read again from scratch: its old
    // claims stay excluded, and `pending` is what gets it fresh ones.
    if (previous && previous.sha256 === file.sha256 && previous.state !== 'excluded') {

 ...
 *
 * The installed payload under `_dev-workflow/` must work in any project — including
...
export function sh(cmd, args = [], opts = {}) {
  return new Promise((resolve) => {
    ...
    // Optional stdin. Long values — an issue body, a comment — are passed this
    // way rather than as an argument: argv has a length limit (ARG_MAX) that a
    // real ticket description can approach, and `--body-file -` is the
    // documented path for the CLIs that accept one.
    if (opts.input !== undefined) {
      child.stdin.on('error', () => {
        // The child may exit before reading stdin (a bad flag, say). That
        // surfaces through the exit code below; an EPIPE here would just be a
        // less useful way to hear about the same failure.
      });

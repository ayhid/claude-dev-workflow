...
function findings(stdout) {
  ...
  // Not re-ordered. Two runs over one unchanged file already print the same
  // bytes, because the linter is deterministic — and sorting these lexically
  // put line 10 before line 2, which is worse than useless in a list a reader
  // is meant to work down.

...
# The escape is configured as a single literal, e.g. `chore(no-ticket)`, but the
# type in it is incidental — `no-ticket` is the part that means "this work has no
# issue". Pinning the type to `chore` makes every ticketless commit a chore, which
# is exactly the type conventional-commits treats as non-releasing: a repo leaning
# on the hatch can never cut a feature release. So accept the same scope under any
# configured type, `!` included, and leave a scope-less escape literal-only.
#
# A regex metacharacter in the scope can only make this match fail, which falls
# through to the normal ticket check below — the safe direction.
escape_scope=${escape#*\(}
escape_scope=${escape_scope%\)}
if [ "$escape_scope" != "$escape" ] && [ -n "$escape_scope" ]; then
  [[ "$subject" =~ ^($types)\($escape_scope\)!?:[[:space:]] ]] && exit 0
fi

...
edit a file, never switch branches — the session may be mid-task on another ticket.
...
priorities, and the repo layout. If it reports `MISSING`, run `/yt-init` first and stop.
...
At most **5 questions, in one batch**, and only ones neither the description nor the code

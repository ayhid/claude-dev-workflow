...
3. **Reviewer** to request on pull requests, if there is a habitual one.

### YouTrack only

4. **Instance URL** and **project key** (the short prefix in issue IDs, e.g. `RMB`).
5. **Token source** — `$YOUTRACK_TOKEN`, or a 1Password ref (`op://Vault/item/credential`).
...
4. **Which repository holds the issues** — `owner/name`. If the project spans several repos this is
   required and not a default: `#123` means a different issue in every repository.
5. **Which label marks each rung.** GitHub has no state field, so the ladder is modelled with
...
6. **The first rung needs no label.** A GitHub ladder must start with a rung meaning "untouched" —
